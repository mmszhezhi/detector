  ...
  public synchronized byte[] getImageData() {
    if (jpegBytes == null) {
      try (ByteArrayOutputStream stream = new ByteArrayOutputStream()) {
          Bitmap bmBackground = Bitmap.createBitmap(image.getBitmap());
          bmBackground.eraseColor(Color.BLACK);
          Bitmap bmObj = getBitmap();
          new Canvas(bmBackground).drawBitmap(bmObj, bmBackground.getWidth() / 2 - bmObj.getWidth() / 2, bmBackground.getHeight() / 2 - bmObj.getHeight() / 2, new Paint());
        bmBackground.compress(CompressFormat.JPEG, /* quality= */ 100, stream);
