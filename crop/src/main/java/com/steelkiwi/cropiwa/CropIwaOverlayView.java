...
public class CropIwaOverlayView extends View implements ConfigChangeListener, OnImagePositionedListener {
    ...
    @Override
    public void onImagePositioned(RectF imageRect, RectF imageCropRect, RectF coordinateSystem, Bitmap bitmap) {
        imageBounds.set(imageRect);
        if (bitmap != null && !flag) {
            if (imageCropRect != null) {
                CropArea cropArea = CropArea.create(coordinateSystem, coordinateSystem, getCropRect());
                RectF newRect = cropArea.convertFromOrigin(imageCropRect, bitmap);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (onDrawListener != null)
            onDrawListener.onDrawCompleted();
    }
